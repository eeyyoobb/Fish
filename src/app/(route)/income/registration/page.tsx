...
export default function SignUpPage() {
  ...
  useEffect(() => {
    ...
    if (referral) {
      setInvitationLabel(referral);
    }
  }, []);

  // Redirect to /income/start if the user is signed in
  useEffect(() => {
    if (isSignedIn) {
      router.push('/income/start');
    }
  }, [isSignedIn, router]);
      ...
      <SignUp.Root>
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {/* New Invitation Label Field */}
            <Clerk.Field name="invitation" className="space-y-2">
              <Clerk.Label className="text-sm font-medium ">Parent</Clerk.Label>
              <Clerk.Input
                type="text"
                value={invitationLabel}
                readOnly // Make it read-only if you don't want users to edit it
                className="w-full rounded-md glass px-3.5 py-2 text-sm outline-none ring-1 ring-inset ring-zinc-300 hover:ring-zinc-400 focus:ring-[1.5px] focus:ring-zinc-950"
              />
              <Clerk.FieldError className="block text-sm text-red-400" />
            </Clerk.Field>
          </div>
